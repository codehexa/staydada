...
function mailer($fname, $fmail, $to, $subject, $content, $type = 0, $file = "", $cc = "", $bcc = "") {
    ...
    $fname = "=?$charset1?B?" . base64_encode($fname) . "?=";
    $subject = "=?$charset1?B?" . base64_encode($subject) . "?=";
    ...
    // UTF-8 관련 수정
    $header .= "X-Mailer: SIR Mailer 0.92 (sir.co.kr) : $_SERVER[SERVER_ADDR] : $_SERVER[REMOTE_ADDR] : $url1 : $_SERVER[PHP_SELF] : $_SERVER[HTTP_REFERER] \n";

    if ($file != "") {
        $boundary = uniqid("http://sunjoy.kr/");
    ...
    if ($type) {
        $header .= "Content-Type: TEXT/HTML; charset=$charset1\n";
        if ($type == 2) $content = nl2br($content);
    } else {
        $header .= "Content-Type: TEXT/PLAIN; charset=$charset1\n";
...
function verify_email($address, &$error) {
    ...
    foreach($weighted_host as $host) {
        ...
        $stoptime = $server_time + $WAIT_SECOND;
        $gotresponse = false;
