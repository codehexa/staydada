...
?>
<script src="//dapi.kakao.com/v2/maps/sdk.js?appkey=<?php echo _KKO_APP_JS_KEY ?>&libraries=services"></script>
<script>
            ...
            oncomplete: function(data) {
                ...
                geocoder.addressSearch(data.address, function(results, status) {
                    ...
                    if (status === daum.maps.services.Status.OK) {
						...
						$("#lon").val(result.x); //경도(longitude)
						$("#lat").val(result.y); //위도(latitude)

                        
                    }
                });

				//console.log(addr_jibun);
				//console.log(addr_road);
                // 우편번호와 주소 정보를 해당 필드에 넣는다.
                document.getElementById('sample_postcode').value = data.zonecode; //5자리 새우편번호 사용
                document.getElementById('sample_address').value = addr_jibun;
				document.getElementById('sample_detailAddress').value = addr_road;
